...
function generateLlmsTxt(config: AeorankVitePressConfig): string {
...
function generateLlmsFullTxt(config: AeorankVitePressConfig): string {
	const faqSection =
...
function generateClaudeMd(config: AeorankVitePressConfig): string {
	return `# ${config.siteName}
...
- VitePress
- AEOrank plugin for AI Engine Optimization
...
function generateSchemaJson(config: AeorankVitePressConfig): string {
...
function generateFaqBlocksHtml(config: AeorankVitePressConfig): string {
...
function generateCitationAnchorsHtml(config: AeorankVitePressConfig): string {
	return `<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
...
function generateSitemapAiXml(config: AeorankVitePressConfig): string {
...
const generators: Record<string, (config: AeorankVitePressConfig) => string> = {
 ...
 */
export function generateAeoFiles(config: AeorankVitePressConfig): string[] {
	const outputDir = config.outputDir ?? ".vitepress/dist";
	ensureDir(outputDir);
