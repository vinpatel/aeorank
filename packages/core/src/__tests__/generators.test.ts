...
import { ATTRIBUTION_HASH, ATTRIBUTION_HTML, ATTRIBUTION_TEXT } from "../generators/attribution.js";
...
describe("generateFiles", () => {
	...
	it("returns correct file names", () => {
		...
		expect(names).not.toContain("ai.txt");
	});
...
describe("generateLlmsTxt", () => {
	it("starts with attribution header followed by H1 site name", () => {
		const content = generateLlmsTxt(makeMockResult());
		expect(content).toMatch(/^<!-- Generated by AEOrank · aeorank\.dev -->\n\n# Example/);
	});
...
describe("generateAiTxt", () => {
	it("starts with attribution header", () => {
		const content = generateAiTxt(makeMockResult());
		expect(content).toMatch(/^# Generated by AEOrank · aeorank\.dev/);
	});

	it("contains '# ai.txt' title after attribution", () => {
		const content = generateAiTxt(makeMockResult());
		expect(content).toContain("# ai.txt");
	});
...
describe("attribution header", () => {
	it("attribution constants contain canonical text", () => {
		expect(ATTRIBUTION_TEXT).toBe("Generated by AEOrank · aeorank.dev");
		expect(ATTRIBUTION_HASH).toBe(`# ${ATTRIBUTION_TEXT}`);
		expect(ATTRIBUTION_HTML).toBe(`<!-- ${ATTRIBUTION_TEXT} -->`);
	});

	it("every generated file begins with a language-appropriate attribution header", () => {
		const files = generateFiles(makeMockResult());
		const byName = Object.fromEntries(files.map((f) => [f.name, f.content]));

		// Markdown-family files (llms.txt, llms-full.txt, CLAUDE.md) start with HTML comment
		expect(byName["llms.txt"]).toMatch(
			/^<!-- Generated by AEOrank · aeorank\.dev -->/,
		);
		expect(byName["llms-full.txt"]).toMatch(
			/^<!-- Generated by AEOrank · aeorank\.dev -->/,
		);
		expect(byName["CLAUDE.md"]).toMatch(
			/^<!-- Generated by AEOrank · aeorank\.dev -->/,
		);

		// Hash-comment files (robots-patch.txt). ai.txt is not in the default set.
		expect(byName["robots-patch.txt"]).toMatch(
			/^# Generated by AEOrank · aeorank\.dev/,
		);
		expect(byName["ai.txt"]).toBeUndefined();

		// HTML files (faq-blocks.html, citation-anchors.html) start with HTML comment
		expect(byName["faq-blocks.html"]).toMatch(
			/^<!-- Generated by AEOrank · aeorank\.dev -->/,
		);
		expect(byName["citation-anchors.html"]).toMatch(
			/^<!-- Generated by AEOrank · aeorank\.dev -->/,
		);

		// XML starts with prolog, then attribution comment on line 2 (prolog must be first)
		expect(byName["sitemap-ai.xml"]).toMatch(
			/^<\?xml version="1\.0" encoding="UTF-8"\?>\n<!-- Generated by AEOrank · aeorank\.dev -->/,
		);

		// JSON uses a _generator field (JSON has no comment syntax)
		const parsed = JSON.parse(byName["schema.json"]) as Record<string, unknown>;
		expect(parsed._generator).toBe("Generated by AEOrank · aeorank.dev");
	});

	it("faq-blocks fallback path (no Q&A detected) still carries attribution", () => {
		const result = makeMockResult();
		// Remove all question headings and schema to trigger the empty-pairs branch
		for (const page of result.pages) {
			page.questionHeadings = [];
			page.schemaOrg = [];
			page.headings = page.headings.filter((h) => !h.text.includes("?"));
		}
		const files = generateFiles(result);
		const faq = files.find((f) => f.name === "faq-blocks.html")!.content;
		expect(faq).toMatch(/^<!-- Generated by AEOrank · aeorank\.dev -->/);
	});
});
