...
import type { AeorankRemixConfig } from "./types.js";
...
function generateLlmsTxt(config: AeorankRemixConfig): string {
...
function generateLlmsFullTxt(config: AeorankRemixConfig): string {
	const faqSection =
...
function generateClaudeMd(config: AeorankRemixConfig): string {
	return `# ${config.siteName}
...
This Remix site publishes the following AEO (AI Engine Optimization) files:
...
function generateSchemaJson(config: AeorankRemixConfig): string {
...
function generateFaqBlocksHtml(config: AeorankRemixConfig): string {
...
function generateCitationAnchorsHtml(config: AeorankRemixConfig): string {
	return `<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
...
function generateSitemapAiXml(config: AeorankRemixConfig): string {
...
const generators: Record<string, (config: AeorankRemixConfig) => string> = {
 ...
 */
export function generateAeoFiles(config: AeorankRemixConfig): string[] {
