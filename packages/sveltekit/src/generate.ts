...
import type { AeorankSvelteKitConfig } from "./types.js";
...
function generateLlmsTxt(config: AeorankSvelteKitConfig): string {
...
function generateLlmsFullTxt(config: AeorankSvelteKitConfig): string {
	const faqSection =
...
function generateClaudeMd(config: AeorankSvelteKitConfig): string {
	return `# ${config.siteName}
...
This SvelteKit site publishes the following AEO (AI Engine Optimization) files:
...
function generateSchemaJson(config: AeorankSvelteKitConfig): string {
...
function generateFaqBlocksHtml(config: AeorankSvelteKitConfig): string {
...
function generateCitationAnchorsHtml(config: AeorankSvelteKitConfig): string {
	return `<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
...
function generateSitemapAiXml(config: AeorankSvelteKitConfig): string {
...
const generators: Record<string, (config: AeorankSvelteKitConfig) => string> = {
 ...
 */
export function generateAeoFiles(config: AeorankSvelteKitConfig): string[] {
	const outputDir = config.outputDir ?? "static";
	ensureDir(outputDir);
