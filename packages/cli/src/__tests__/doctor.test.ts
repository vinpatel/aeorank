...
describe("doctor command", () => {
	...
	it("does not flag 404 /llms.txt or 405 HEAD responses as firewall blocks", async () => {
		...
		for (const crawler of report.crawlers) {
			...
			expect(crawler.target.status).toBe(405);
			expect(crawler.llmsTxt.status).toBe(404);
			expect(crawler.target.wafSignals).toHaveLength(0);
			expect(crawler.llmsTxt.wafSignals).toHaveLength(0);
		}
	});

	it("flags AWS WAF CAPTCHA (405 + x-amzn-waf-action) and leaves a bare 405 alone", async () => {
		const report = await runDoctor("https://example.com", {
			fetchImpl: makeFetchMock({
				robots: "User-agent: *\nAllow: /",
				targetByUserAgent: {
					GPTBot: { status: 405, headers: { "x-amzn-waf-action": "captcha" } },
				},
				defaultTarget: { status: 405 },
				defaultLlms: { status: 404 },
			}),
		...
		expect(gptBot?.target.blocked).toBe(true);
		expect(gptBot?.target.wafSignals).toEqual([{ header: "x-amzn-waf-action", value: "captcha" }]);
		expect(claudeBot?.target.blocked).toBe(false);
		expect(claudeBot?.target.status).toBe(405);
		expect(report.verdict.status).toBe("fail");
	});

	it("treats missing robots.txt as unspecified and does not fail the verdict", async () => {
		const report = await runDoctor("https://example.com", {
			fetchImpl: makeFetchMock({
				robots: null,
		...
		expect(report.crawlers).toHaveLength(AI_CRAWLERS.length);
		for (const crawler of report.crawlers) {
			expect(crawler.robots).toBe("unspecified");
			expect(crawler.target.blocked).toBe(false);
			expect(crawler.llmsTxt.blocked).toBe(false);
		}
	...
	it("prints JSON from the command action", async () => {
		...
		expect(parsed.crawlers.map((crawler: { userAgent: string }) => crawler.userAgent)).toEqual([
			...AI_CRAWLERS,
		]);
	});

	it("prints a JSON error and exits 1 for an invalid URL", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		const exitSpy = vi.spyOn(process, "exit").mockImplementation((() => {}) as never);

		await doctorCommand.parseAsync(["node", "doctor", "not-a-url", "--format", "json"]);

		expect(exitSpy).toHaveBeenCalledWith(1);
		const parsed = JSON.parse(String(logSpy.mock.calls.at(-1)?.[0]));
		expect(parsed.error).toContain("not a valid URL");
	...
	it("rejects a non-positive timeout", async () => {
		...
		await doctorCommand.parseAsync(["node", "doctor", "https://example.com", "--timeout", "0"]);
