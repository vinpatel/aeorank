...
/**
 * Access-control statuses. 404 (missing `/llms.txt`) and 405 (HEAD not
 * implemented) are omitted on purpose. AWS WAF CAPTCHA still counts: it
 * answers 405 together with `x-amzn-waf-action: captcha`.
 */
const BLOCK_STATUSES = new Set([401, 403, 406, 407, 418, 429, 451, 503, 999]);

/**
 * Header names confirmed in vendor docs:
 * - Cloudflare `cf-mitigated` (challenge pages; documented value `challenge`)
 * - AWS WAF `x-amzn-waf-action` (`challenge` on 202, `captcha` on 405)
 */
	...
	.action(async (url: string, options: DoctorCommandOptions) => {
		...
		try {
			...
			try {
				...
				throw new AeorankError(`'${url}' is not a valid URL.`, suggestion);
			}

			spinner.start();
			const report = await runDoctor(parsedUrl.toString(), { timeoutMs });
			spinner.stop();
...
/**
 * Probe AI crawler access without running the 12-dimension scan.
 * Robots allow/block/unknown comes from `parseRobotsTxt` (site root).
 * A finished report exits 0; the verdict lives in `report.verdict`.
 */
...
function buildVerdict(crawlers: CrawlerDoctorResult[]): DoctorReport["verdict"] {
	...
	if (blockedCrawlers > 0) {
		const summary =
			blockedCrawlers === 1
				? "1 crawler looks blocked."
				: `${blockedCrawlers} crawlers look blocked.`;
		...
		.map((row, rowIndex) => {
			const line = row
				.map((cell, columnIndex) => {
					...
					return cell + " ".repeat(width - stripAnsi(cell).length);
				})
				.join("  ");
